...
erroring — an unknown container renders its children without its own
markup, and a node whose content lives only in its attributes renders
nothing. A custom editor node with no rule doesn't break the render; it
quietly goes missing from the HTML. So when you add editor nodes, add a
test: render a real document and assert `unknown_types` is empty.
