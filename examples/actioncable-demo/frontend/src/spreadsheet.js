// Opaque-state demo: a collaborative spreadsheet.
// Shared state is a Y.Array of row Y.Maps, keyed by column id. A cell is not a
// scalar — it is its own Y.Map of { value, bold, fill }, so bolding a cell and
// typing in it are writes to different keys and both survive. Same property
// twice is last-writer-wins. Sorting and column order come from TanStack Table
// and are deliberately NOT shared: they are this browser's view of the same
// rows. yrby knows nothing about "cells" or "columns" either way.
...
window.__yrb = { provider, ydoc, rows, user, newRow, getCell, addRow }
...
rows.observeDeep(() => { table.setOptions((old) => ({ ...old, data: snapshot() })); render() })
...
// --- Presence -----------------------------------------------------------------
// Each peer publishes the cell it is focused in; that cell gets their color and
...
function renderPresence() {
  ...
  presenceEl.innerHTML = states.map(([, s]) => s.user).filter(Boolean)
